...
export function createFilterString(filter) {
    ...
    return decodeURIComponent(
        new URLSearchParams(searchFilter).toString()
    ).toString();
}

export async function getDataWithFilter(filter) {
    let filterString = createFilterString(filter);
    var data = await (await fetch("/api/stocks?" + filterString)).json();
    const sortedCars = sortCars(data.stocks, filter);
    data.stocks = sortedCars;
    return data;
}
